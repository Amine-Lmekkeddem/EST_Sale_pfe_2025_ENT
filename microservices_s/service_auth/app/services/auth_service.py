...
def decode_token(token: str):
    try:
        ...
        user_info = {
            "username": payload.get("preferred_username"),
            "email": payload.get("email"),
            "role": main_role,  # Utilise le rôle principal filtré
            "user_id": payload.get("sub")
        }
        logger = logging.getLogger(__name__)
        logger.info(f"Informations extraites du token : {user_info}")
        return user_info
...
async def handle_user_login(user_info: dict):
    user_id = UUID(user_info["user_id"])  # Conversion en UUID dès le départ
    
    if not await check_user_exists(user_id):
        await insert_user(user_info)
        print(f"✅ Utilisateur {user_info['username']} ajouté dans Cassandra")
    else:
        print(f"ℹ️ Utilisateur {user_info['username']} existe déjà dans Cassandra")
